        ...
        private void modLevelUpdate(On.Celeste.Level.orig_Update orig, Level self)
        {
            ...
            if (XaphanModule.useUpgrades)
            {
                ...
                if (isActive && !XaphanModule.PlayerIsControllingRemoteDrone() && !GravityJacket.determineIfInWater())
                {
                    ...
                    if (self.CanPause && !XaphanModule.PlayerIsControllingRemoteDrone() && player != null && player.StateMachine.State == Player.StNormal && !player.Ducking && !self.Session.GetFlag("In_bossfight") && !self.Session.GetFlag("XaphanHelper_Prevent_Drone") && XaphanModule.ModSettings.UseBagItemSlot.Pressed && !XaphanModule.ModSettings.OpenMap.Check && !XaphanModule.ModSettings.SelectItem.Check && !self.Session.GetFlag("Map_Opened") && player.Holding == null && !UseDroneCoroutine.Active)
                    {
